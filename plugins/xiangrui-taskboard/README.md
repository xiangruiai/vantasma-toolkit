...
首次打开成功后，Agent 会询问一次是否愿意赞赏。可以直接跳过；如果告诉 Agent 金额，它会在当前对话中生成微信支付链接，不需要再跳转到赞赏页。赞赏不会解锁任何功能，也不影响免费使用。
...
直接告诉 Agent“我想赞赏 10 元”，它会调用服务端微信支付接口并返回本次订单的支付链接。金额支持 1 至 2000 元，支付完全自愿，也可以随时跳过。
