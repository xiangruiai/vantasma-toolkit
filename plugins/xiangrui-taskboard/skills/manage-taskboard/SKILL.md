...
description: Manage Xiangrui Taskboard projects, issues, relations, and comments. Use when Codex needs to record a requirement, inspect project work, create or update issues, add progress notes, begin execution, hand off review, or coordinate concurrent task updates.
---

# Manage Taskboard

Use the plugin-bundled `taskctl` wrapper for every project, issue, and comment operation. Resolve the plugin root from this file (`../..`) and invoke `node <plugin-root>/scripts/taskctl.mjs ...`. Read [references/cli.md](references/cli.md) before choosing a command or option. When that reference shows `taskctl`, substitute the bundled wrapper command.
