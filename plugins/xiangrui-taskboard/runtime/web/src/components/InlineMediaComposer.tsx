...
interface InlineMediaComposerProps {
  ...
  disabled?: boolean;
  className?: string;
  imagePreviewLayout?: "inline" | "strip";
...
export const InlineMediaComposer = forwardRef<InlineMediaComposerHandle, InlineMediaComposerProps>(
  function InlineMediaComposer({
    ...
    const pendingImages = inlineMediaImages(segments);
    const visibleTextIds = new Set(
      imagePreviewLayout === "strip"
        ? segments
            .filter((segment): segment is InlineTextSegment => segment.type === "text")
            .filter((segment, index, textSegments) => (
              segment.text.length > 0 || index === textSegments.length - 1
            ))
            .map((segment) => segment.id)
        : [],
    );
    const firstVisibleTextId = imagePreviewLayout === "strip"
      ? segments.find((segment) => segment.type === "text" && visibleTextIds.has(segment.id))?.id
      : undefined;

    return (
      <div className={`inline-media-composer ${className}`.trim()} aria-label={ariaLabel}>
        {imagePreviewLayout === "strip" && pendingImages.length > 0 && (
          <div className="inline-media-image-strip" aria-label="待上传图片">
            {pendingImages.map((segment) => (
              <PendingImageBlock
                key={segment.id}
                segment={segment}
                disabled={disabled}
        ...
        {segments.map((segment, index) => (
          segment.type === "text" ? (
            imagePreviewLayout === "strip" && !visibleTextIds.has(segment.id) ? null :
            <textarea
              key={segment.id}
              ref={(element) => {
              ...
              placeholder={(
                imagePreviewLayout === "strip"
                  ? segment.id === firstVisibleTextId && inlineMediaText(segments).length === 0
                  : isEmpty && index === 0
