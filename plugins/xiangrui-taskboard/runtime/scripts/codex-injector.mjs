        ...
        try {
          const readiness = await cdp.send("Runtime.evaluate", {
            expression: `({
              ...
              injectionReady: typeof window.__codexTaskboardInjection__?.refresh === "function",
              sourceHash: window.__codexTaskboardInjection__?.sourceHash || null
            })`,
            returnByValue: true,
          });
          if (isResidentInjectionReady(readiness.result.value, {
            token: startupToken,
...
async function finishPreparedTaskComposerViaCdp(
  ...
  while (Date.now() < deadline) {
    ...
    if (submitted.result.value === true) return { prefilled: true, submitted: true };
...
async function prefillTaskComposerContentsViaCdp(cdp, executionContextId, request) {
  ...
  while (Date.now() < editorDeadline) {
    ...
    if (prepared.result.value.matches) {
      return finishPreparedTaskComposerViaCdp(
        cdp,
        executionContextId,
        instruction,
        autoSubmit,
