...
taskctl project update PROJECT_ID --name NAME [--json]
...
taskctl issue create \
  ...
  [--blocked-reason TEXT --unblock-action TEXT] \
  [--json]
```
...
Creating or moving an issue to `blocked` requires both `--blocked-reason` and `--unblock-action`. The reason must be an explicit confirmed fact from a person or a verified tool/runtime error. The unblock action must say exactly what needs to happen before work can resume. `taskctl` rejects missing, blank, or partially supplied blocking information; do not derive it from comments or status.
...
taskctl comment add ISSUE_ID --body TEXT [--reply-to COMMENT_ID] [--thread-id ID] [--json]
...
Use `--reply-to COMMENT_ID` when an agent result answers a specific issue comment. The referenced comment must belong to the same issue, and the UI will show the relationship as a link back to the source feedback.
