...
const COMMAND_OPTIONS = new Map([
  ...
  ["project update", new Set(["name", "json"])],
  ...
  ["comment add", new Set(["body", "reply-to", "thread-id", "json"])],
...
async function execute(parsed, overrides) {
  ...
  if (!allowedOptions) {
    throw usageError(
      "Expected one of: project list/create/update/map, cloud login/status/logout, issue list/get/create/update/move/archive/restore/relation, comment list/add/update/delete, attachment download, context current",
    );
  }
  ...
  switch (command) {
    ...
    case "project update":
      ...
      return api.request(
        "PATCH",
        `/api/projects/${encodeURIComponent(parsed.operands[0])}`,
        { name: requiredOption(parsed.options, "name") },
      );
    case "project map":
      expectOperandCount(parsed, 1);
    ...
    case "comment add":
      ...
      return api.request("POST", `${taskPath(parsed.operands[0])}/comments`, {
        ...
        ...optionalField("replyToCommentId", parsed.options["reply-to"]),
...
async function createIssue(api, options, overrides) {
  ...
  const blocking = blockingFromOptions(status, options);
  ...
  return api.request("POST", "/api/tasks", {
    ...
    ...optionalField("blocking", blocking),
  });
}
...
async function updateIssue(api, taskId, options, overrides) {
  ...
  const developmentContext = developmentContextFromOptions(options, overrides);
  const recurrence = recurrenceFromOptions(options);
  const blocking = blockingFromOptions(options.status, options);
  const threadId = resolveThreadId(options, overrides);
  const patch = {
...
function blockingFromOptions(status, options) {
  const rawReason = options["blocked-reason"];
  const rawUnblockAction = options["unblock-action"];
  const hasBlockingOption = rawReason !== undefined || rawUnblockAction !== undefined;

  if (status !== "blocked") {
    if (hasBlockingOption) {
      throw usageError("--blocked-reason and --unblock-action require --status blocked");
    }
    return undefined;
  }

  const reason = typeof rawReason === "string" ? rawReason.trim() : "";
  const unblockAction = typeof rawUnblockAction === "string" ? rawUnblockAction.trim() : "";
  if (!reason || !unblockAction) {
    throw usageError(
      "Moving an issue to blocked requires --blocked-reason and --unblock-action",
    );
  }
  return { reason, unblockAction };
}
