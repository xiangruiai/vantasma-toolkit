...
function taskFromRow(row) {
  ...
  return {
    ...
    status: row.status,
    blocking: row.status === "blocked" && row.blocked_reason
      ? {
          reason: row.blocked_reason,
          unblockAction: row.unblock_action,
          recordedBy: {
            type: row.blocked_by_type,
            id: row.blocked_by_id,
            name: row.blocked_by_name,
            avatarUrl: row.blocked_by_avatar_url,
          },
          recordedAt: row.blocked_at,
        }
      : null,
...
export class TaskboardDatabase {
  ...
  #migrate() {
    this.database.exec(`
      ...
      CREATE TABLE IF NOT EXISTS tasks (
        ...
        unblock_action TEXT,
        blocked_at TEXT,
        blocked_by_type TEXT CHECK (blocked_by_type IN ('user', 'agent')),
        blocked_by_id TEXT,
        blocked_by_name TEXT,
        blocked_by_avatar_url TEXT,
      ...
      CREATE TABLE IF NOT EXISTS comments (
        ...
        body TEXT NOT NULL,
        thread_id TEXT,
        reply_to_comment_id TEXT REFERENCES comments(id) ON DELETE SET NULL,
    ...
    if (!taskColumns.some((column) => column.name === "blocked_reason")) {
      this.database.exec("ALTER TABLE tasks ADD COLUMN blocked_reason TEXT");
    }
    if (!taskColumns.some((column) => column.name === "unblock_action")) {
      this.database.exec("ALTER TABLE tasks ADD COLUMN unblock_action TEXT");
    }
    if (!taskColumns.some((column) => column.name === "blocked_at")) {
      this.database.exec("ALTER TABLE tasks ADD COLUMN blocked_at TEXT");
    }
    if (!taskColumns.some((column) => column.name === "blocked_by_type")) {
      this.database.exec("ALTER TABLE tasks ADD COLUMN blocked_by_type TEXT CHECK (blocked_by_type IN ('user', 'agent'))");
    }
    if (!taskColumns.some((column) => column.name === "blocked_by_id")) {
      this.database.exec("ALTER TABLE tasks ADD COLUMN blocked_by_id TEXT");
    }
    if (!taskColumns.some((column) => column.name === "blocked_by_name")) {
      this.database.exec("ALTER TABLE tasks ADD COLUMN blocked_by_name TEXT");
    }
    if (!taskColumns.some((column) => column.name === "blocked_by_avatar_url")) {
      this.database.exec("ALTER TABLE tasks ADD COLUMN blocked_by_avatar_url TEXT");
    }
    ...
    if (!commentColumns.some((column) => column.name === "reply_to_comment_id")) {
      this.database.exec(
    ...
    try {
      this.database.exec(`
        ...
        INSERT INTO tasks_status_migration (
          ...
          sort_order, thread_id, blocked_reason, unblock_action, blocked_at,
          blocked_by_type, blocked_by_id, blocked_by_name, blocked_by_avatar_url,
  ...
  createTask(input) {
    ...
    try {
      ...
      `).run(
        ...
        sortOrder,
        input.threadId ?? null,
        input.blocking?.reason ?? null,
        input.blocking?.unblockAction ?? null,
        input.blocking ? timestamp : null,
        input.blocking ? input.actor.type : null,
        input.blocking ? input.actor.id : null,
        input.blocking ? input.actor.name : null,
        input.blocking ? input.actor.avatarUrl : null,
        input.actor.type,
        input.actor.id,
        input.actor.name,
  ...
  updateTask(id, version, changes, threadId) {
    ...
    const enteringBlocked = current.status !== "blocked" && nextStatus === "blocked";
    if (enteringBlocked && !changes.blocking) {
      throw new ApiError(
        400,
        "BLOCKING_INFO_REQUIRED",
        "Moving an issue to blocked requires an exact reason and unblock action",
      );
    }
    if (changes.blocking && nextStatus !== "blocked") {
      throw new ApiError(400, "INVALID_FIELD", "'blocking' is only valid when status is blocked");
    }
    ...
    for (const [key, value] of Object.entries(changes)) {
      ...
      if (key === "blocking") {
        assignments.push(
          ...
          "blocked_by_id = ?",
          "blocked_by_name = ?",
          "blocked_by_avatar_url = ?",
        );
        values.push(
          value.reason,
          value.unblockAction,
          now(),
          value.recordedBy.type,
          value.recordedBy.id,
          value.recordedBy.name,
          value.recordedBy.avatarUrl,
        );
        continue;
      }
  ...
  moveTask(id, version, status, sortOrder, threadId, blocking, actor) {
    const current = this.#requireTask(id);
    this.#requireVersion(current, version);
    if (current.archivedAt !== null) {
      throw new ApiError(409, "TASK_ARCHIVED", "Archived tasks cannot be moved");
    }
    const enteringBlocked = current.status !== "blocked" && status === "blocked";
    if (enteringBlocked && !blocking) {
      throw new ApiError(
        400,
        "BLOCKING_INFO_REQUIRED",
        "Moving an issue to blocked requires an exact reason and unblock action",
    ...
    this.database.exec("BEGIN IMMEDIATE");
    try {
      const blockingAssignments = blocking
        ? `,
          blocked_reason = ?, unblock_action = ?, blocked_at = ?,
          blocked_by_type = ?, blocked_by_id = ?, blocked_by_name = ?, blocked_by_avatar_url = ?`
        : "";
      const result = this.database.prepare(`
        UPDATE tasks
        SET status = ?, sort_order = ?, thread_id = COALESCE(?, thread_id)${blockingAssignments},
  ...
  createComment(taskId, input) {
    ...
    if (input.replyToCommentId) {
      const replyTarget = this.#requireComment(input.replyToCommentId);
      if (replyTarget.taskId !== task.id) {
        throw new ApiError(400, "INVALID_REPLY_TARGET", "Reply target must belong to the same issue");
      }
    }
    const id = randomUUID();
    const timestamp = now();
    this.database.prepare(`
      INSERT INTO comments (
        id, task_id, body, thread_id, reply_to_comment_id,
