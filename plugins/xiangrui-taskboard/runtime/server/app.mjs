...
function parseBlocking(value) {
  ...
  return {
    reason: stringField(value.reason, "blocking.reason", { required: true, maxLength: 4_000 }),
...
function parseTaskCreate(body) {
  ...
  assertAllowedKeys(body, new Set([
    ...
    "assigneeTarget", "workflowId", "developmentContext", "dueDate", "recurrence", "blocking",
  ]));
  ...
  if (task.status === "blocked" && !task.blocking) {
    throw new ApiError(
      400,
...
function parseCommentCreate(body) {
  assertPlainObject(body);
  assertAllowedKeys(body, new Set(["body", "threadId", "replyToCommentId"]));
  return {
    body: stringField(body.body ?? "", "body", { maxLength: 100_000 }),
    threadId: parseThreadId(body.threadId),
    replyToCommentId: body.replyToCommentId === undefined
      ? undefined
      : stringField(body.replyToCommentId, "replyToCommentId", { required: true, maxLength: 128 }),
  };
}
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      if (projectRoute) {
        ...
        validateProjectId(projectId);
        if (request.method === "PATCH") {
          const input = parseProjectUpdate(await readJson(request));
          const project = database.updateProjectName(projectId, input.name);
      ...
      if (taskRoute) {
        ...
        if (!action && request.method === "PATCH") {
          ...
          if (changes.blocking) changes.blocking.recordedBy = actor;
