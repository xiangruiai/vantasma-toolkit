...
test("the stable name and generated prompt are project-scoped and encode the claim protocol", () => {
  ...
  assert.match(prompt, /threadId.*failed.*interrupted/);
  assert.match(prompt, /禁止仅根据 updatedAt/);
  assert.match(prompt, /assignee\.type=agent.*assignee\.id=codex-agent/);
  assert.match(prompt, /blockedBy.*status!=done/);
  assert.match(prompt, /priority.*dueDate.*sortOrder.*createdAt/);
  assert.match(prompt, /版本冲突.*跳过/);
  assert.match(prompt, /关键改动、验证结果、执行结果和剩余风险/);
  assert.match(prompt, /in_review/);
  assert.match(prompt, /中断点并退回 todo/);
  assert.match(prompt, /解除条件并移到 blocked/);
  assert.match(prompt, /--blocked-reason/);
  assert.match(prompt, /--unblock-action/);
  assert.match(prompt, /不得根据评论或状态推断/);
  assert.match(prompt, /不得直接标记为 done/);
