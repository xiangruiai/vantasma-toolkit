...
test("project update renames an existing project without changing its id", async () => {
  let requestedUrl;
  let requestMethod;
  let requestBody;
  const result = await run(
...
test("issue commands accept in-review, blocked, and canceled statuses", async () => {
  ...
  assert.deepEqual(moveBody.blocking, {
    reason: "等待测试环境恢复",
    unblockAction: "恢复测试环境并通知执行者",
  });
...
test("issue move sends the exact blocking reason and unblock action", async () => {
  let requestBody;
  const result = await run(
    [
      "issue",
  ...
  assert.equal(result.exitCode, 0);
  assert.deepEqual(requestBody.blocking, {
    reason: "等待祥瑞确认账户类型",
    unblockAction: "回复普通商户或服务商",
  });
});

test("issue move refuses blocked without a reason and unblock action", async () => {
  const result = await run(
  ...
  assert.match(result.stderr.error.message, /--blocked-reason/);
  assert.match(result.stderr.error.message, /--unblock-action/);
});
