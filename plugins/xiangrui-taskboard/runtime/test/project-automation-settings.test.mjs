...
test("project mapping is based on exact ids and workspace paths, never project names", () => {
  ...
  assert.match(appSource, /normalizedWorkspacePath\(hostContext\.workspacePath\) === normalizedSelectedWorkspace/);
  assert.match(appSource, /currentHostProjectMatchesWorkspace[\s\S]*?hostContext\?\.projectId/);
  assert.match(appSource, /normalizedWorkspacePath\(deviceWorkspacePaths\[project\.id\]\) === normalizedSelectedWorkspace/);
...
test("the project navigation automation menu owns the icon, fields, and accessible popover", () => {
  ...
  assert.match(menuSource, /active \? "play" : "pause"/);
  ...
  assert.doesNotMatch(menuSource, /无自动化/);
  assert.match(menuSource, /项目未映射/);
  assert.match(menuSource, /自动化不可用/);
  assert.match(menuSource, /未启用/);
...
test("the automation menu reuses the Linear switch and keeps form focus chrome suppressed", () => {
  ...
  assert.match(styles, /\.project-automation-field \.taskboard-select-trigger\s*\{[^}]*height:\s*26px;[^}]*background:\s*var\(--surface\)/s);
  assert.doesNotMatch(menuSource, /<select/);
  assert.equal(menuSource.match(/<TaskboardSelect/g)?.length, 3);
...
test("opening settings and changing projects reconcile with the host list", () => {
  ...
  assert.match(appSource, /sendAutomationRequest\([\s\S]*?stored \? "apply-policy" : "list",[\s\S]*?stored\?\.automationId/);
  ...
  assert.match(appSource, /if \(!policy\) return/);
