...
test("cloud blocking requires exact structured information and records its author", async () => {
  await createProject("cloud-blocking");
  const created = await createTask("cloud-blocking", "Needs a verified dependency");

  const rejected = await cloud.request(`/api/tasks/${created.body.task.id}/move`, {
    method: "POST",
    actorName: alice,
    json: { version: created.body.task.version, status: "blocked" },
  });
  assert.equal(rejected.response.status, 400);
  assert.equal(rejected.body.error.code, "BLOCKING_INFO_REQUIRED");
  ...
  const blocked = await cloud.request(`/api/tasks/${created.body.task.id}/move`, {
    ...
    json: {
      ...
      status: "blocked",
      blocking: {
        reason: "等待 Bob 提供生产数据库只读账号",
        unblockAction: "Bob 发送只读账号并确认权限范围",
      },
    },
  });
  ...
  assert.deepEqual(blocked.body.task.blocking, {
    ...
    recordedBy: {
      type: "user",
      id: blocked.body.task.blocking.recordedBy.id,
      name: alice,
      avatarUrl: null,
    },
  ...
  const resumed = await cloud.request(`/api/tasks/${created.body.task.id}/move`, {
    method: "POST",
    actorName: bob,
    json: { version: blocked.body.task.version, status: "todo" },
  });
