...
test("projects can be renamed without changing their id or issue count", async () => {
  const baseUrl = await startServer();
  const createResult = await request(baseUrl, "/api/tasks", {
    method: "POST",
    body: {
      projectId: "local",
      title: "Keep this issue attached",
    },
  });
  assert.equal(createResult.response.status, 201);

  const renameResult = await request(baseUrl, "/api/projects/local", {
    method: "PATCH",
    body: { name: "xiangrui" },
  });

  assert.equal(renameResult.response.status, 200);
  assert.equal(renameResult.body.project.id, "local");
  assert.equal(renameResult.body.project.name, "xiangrui");
  assert.equal(renameResult.body.project.issueCount, 1);

  const listResult = await request(baseUrl, "/api/projects");
  assert.equal(listResult.body.projects[0].id, "local");
  assert.equal(listResult.body.projects[0].name, "xiangrui");
  assert.equal(listResult.body.projects[0].issueCount, 1);
});
...
test("blocking a task requires exact reason and unblock action with recorded provenance", async () => {
  const baseUrl = await startServer();
  const createResult = await request(baseUrl, "/api/tasks", {
    method: "POST",
    body: { title: "Wait for a decision", status: "todo" },
  });
  const task = createResult.body.task;

  const missing = await request(baseUrl, `/api/tasks/${task.id}/move`, {
    method: "POST",
    body: { version: task.version, status: "blocked" },
  });
  assert.equal(missing.response.status, 400);
  assert.equal(missing.body.error.code, "BLOCKING_INFO_REQUIRED");

  const blocked = await request(baseUrl, `/api/tasks/${task.id}/move`, {
    method: "POST",
    headers: {
      "x-taskboard-user-id": "xiangrui",
      "x-taskboard-user-name": encodeURIComponent("祥瑞"),
    },
    body: {
      ...
      blocking: {
        reason: "等待确认微信支付账户类型",
        unblockAction: "回复普通商户或服务商",
      },
    },
  });
  assert.equal(blocked.response.status, 200);
  assert.equal(blocked.body.task.status, "blocked");
  assert.deepEqual(blocked.body.task.blocking, {
    reason: "等待确认微信支付账户类型",
    unblockAction: "回复普通商户或服务商",
    recordedBy: {
      type: "user",
    ...
    recordedAt: blocked.body.task.blocking.recordedAt,
  });
  assert.match(blocked.body.task.blocking.recordedAt, /^\d{4}-\d{2}-\d{2}T/);

  const resumed = await request(baseUrl, `/api/tasks/${task.id}/move`, {
    method: "POST",
    body: { version: blocked.body.task.version, status: "todo" },
  });
  assert.equal(resumed.response.status, 200);
  assert.equal(resumed.body.task.blocking, null);

  const blockedAgainWithoutFreshReason = await request(baseUrl, `/api/tasks/${task.id}/move`, {
    method: "POST",
    body: { version: resumed.body.task.version, status: "blocked" },
  });
  assert.equal(blockedAgainWithoutFreshReason.response.status, 400);
  assert.equal(blockedAgainWithoutFreshReason.body.error.code, "BLOCKING_INFO_REQUIRED");
});
