...
test("composer handoff preserves an explicit auto-submit request", async () => {
  const responses = [];
  const prefillRequests = [];
  const request = {
  ...
  const result = await handleHostBindingPayload(
    { payload: JSON.stringify(request), executionContextId: 13 },
    {
      parseAutomationRequest: () => assert.fail("automation parser must not run"),
      ensure: async () => assert.fail("ensure must not run"),
      runAutomation: async () => assert.fail("automation must not run"),
      prefill: async (parsed, executionContextId) => {
        prefillRequests.push({ parsed, executionContextId });
  ...
  assert.deepEqual(prefillRequests, [{ parsed: request, executionContextId: 13 }]);
  assert.deepEqual(responses, [{
    id: request.id,
    ok: true,
    prefilled: true,
...
test("composer handoff rejects a non-boolean auto-submit flag", async () => {
  const responses = [];
  const result = await handleHostBindingPayload(
    {
...
test("resident readiness does not depend on a sidebar entry that Codex may not render", () => {
  const expected = {
    token: "replacement-token",
    sourceHash: "current-hash",
  };
  assert.equal(isResidentInjectionReady({
    token: expected.token,
    sourceHash: expected.sourceHash,
    injectionReady: true,
    taskboardEntryMounted: false,
  }, expected), true);
  assert.equal(isResidentInjectionReady({
    token: expected.token,
    sourceHash: expected.sourceHash,
    injectionReady: false,
    taskboardEntryMounted: true,
  }, expected), false);
  assert.equal(isResidentInjectionReady({
    token: "stale-token",
    sourceHash: expected.sourceHash,
