...
test("workflow data is preserved while unfinished workflow controls stay hidden", () => {
  ...
  assert.doesNotMatch(editorSource, /<span className="sr-only">工作流<\/span>/);
  assert.doesNotMatch(detailSource, /<span className="detail-property-label">工作流<\/span>/);
  assert.doesNotMatch(editorSource, /ariaLabel="工作流"/);
  assert.doesNotMatch(detailSource, /ariaLabel="工作流"/);
});
