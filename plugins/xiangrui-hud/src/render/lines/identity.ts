...
const DEBUG =
  process.env.DEBUG?.includes("xiangrui-hud") || process.env.DEBUG === "*";
...
export function renderIdentityLine(ctx: RenderContext): string {
  ...
  if (DEBUG && autocompactMode === "disabled") {
    console.error(
      `[xiangrui-hud:context] autocompactBuffer=disabled, showing raw ${rawPercent}% (buffered would be ${bufferedPercent}%)`,
    );
  }
