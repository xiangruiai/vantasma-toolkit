...
async function createTempHome() {
  return await mkdtemp(path.join(tmpdir(), 'xiangrui-hud-speed-'));
}
...
test('getOutputSpeed writes cache under CLAUDE_CONFIG_DIR by default', async () => {
  ...
  try {
    ...
    const customCachePath = path.join(customConfigDir, 'plugins', 'xiangrui-hud', '.speed-cache.json');
    const defaultCachePath = path.join(tempHome, '.claude', 'plugins', 'xiangrui-hud', '.speed-cache.json');
