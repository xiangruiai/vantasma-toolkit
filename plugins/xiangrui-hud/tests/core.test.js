...
async function getTranscriptCacheFile(configDir) {
  const cacheDir = path.join(configDir, 'plugins', 'xiangrui-hud', 'transcript-cache');
...
test('parseTranscript accumulates session token usage from assistant messages', async () => {
  const dir = await mkdtemp(path.join(tmpdir(), 'xiangrui-hud-'));
...
test('parseTranscript does not cache partial results when stream creation fails after file state lookup', async () => {
  const dir = await mkdtemp(path.join(tmpdir(), 'xiangrui-hud-transcript-cache-'));
...
test('countConfigs honors project and global config locations', async () => {
  const homeDir = await mkdtemp(path.join(tmpdir(), 'xiangrui-hud-home-'));
  const projectDir = await mkdtemp(path.join(tmpdir(), 'xiangrui-hud-project-'));
...
// https://github.com/jarrodwatts/xiangrui-hud/issues/3
...
async function getConfigCacheDir(configDir) {
  return path.join(configDir, 'plugins', 'xiangrui-hud', 'config-cache');
}

test('countConfigs cache: second call uses cache (mtime unchanged)', async () => {
  const homeDir = await mkdtemp(path.join(tmpdir(), 'xiangrui-hud-cc-'));
  const originalHome = process.env.HOME;
  process.env.HOME = homeDir;
...
test('countConfigs cache: miss on nested rules additions', async () => {
  const homeDir = await mkdtemp(path.join(tmpdir(), 'xiangrui-hud-cc-'));
  const projectDir = await mkdtemp(path.join(tmpdir(), 'xiangrui-hud-proj-'));
  const originalHome = process.env.HOME;
  process.env.HOME = homeDir;
...
test('countConfigs cache: isolation between different cwds', async () => {
  const homeDir = await mkdtemp(path.join(tmpdir(), 'xiangrui-hud-cc-'));
  const projectA = await mkdtemp(path.join(tmpdir(), 'xiangrui-hud-projA-'));
  const projectB = await mkdtemp(path.join(tmpdir(), 'xiangrui-hud-projB-'));
  const originalHome = process.env.HOME;
  process.env.HOME = homeDir;
