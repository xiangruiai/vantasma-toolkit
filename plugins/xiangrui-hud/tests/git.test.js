...
test('getGitBranch returns null for non-git directory', async () => {
  const dir = await mkdtemp(path.join(tmpdir(), 'xiangrui-hud-nogit-'));
  try {
...
test('getGitBranch returns branch name for git directory', async () => {
  const dir = await mkdtemp(path.join(tmpdir(), 'xiangrui-hud-git-'));
  try {
...
test('getGitStatus builds branchUrl from HTTPS origin remotes', async () => {
  ...
  try {
    ...
    execFileSync('git', ['commit', '--allow-empty', '-m', 'init'], { cwd: dir, stdio: 'ignore' });
    execFileSync('git', ['checkout', '-b', 'feature/test-branch'], { cwd: dir, stdio: 'ignore' });
    execFileSync('git', ['remote', 'add', 'origin', 'https://github.com/example/xiangrui-hud.git'], { cwd: dir, stdio: 'ignore' });

    const result = await getGitStatus(dir);
    assert.equal(result?.branchUrl, 'https://github.com/example/xiangrui-hud/tree/feature/test-branch');
  } finally {
    await rm(dir, { recursive: true, force: true });
  }
});
...
test('getGitStatus builds branchUrl from SSH origin remotes', async () => {
  ...
  try {
    ...
    execFileSync('git', ['commit', '--allow-empty', '-m', 'init'], { cwd: dir, stdio: 'ignore' });
    execFileSync('git', ['checkout', '-b', 'feature/test-branch'], { cwd: dir, stdio: 'ignore' });
    execFileSync('git', ['remote', 'add', 'origin', 'git@github.com:example/xiangrui-hud.git'], { cwd: dir, stdio: 'ignore' });
