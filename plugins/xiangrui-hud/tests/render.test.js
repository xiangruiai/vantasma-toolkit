...
function captureRenderLines(ctx) {
  ...
  // 核心渲染测试关闭品牌标识，品牌单独在 brand.test.js 验证。
  const coreCtx = { ...ctx, config: { ...ctx.config, display: { ...ctx.config?.display, brand: false } } };
  try {
    render(coreCtx);
  } finally {
...
async function withDeterministicSpeedCache(fn) {
  const tempConfigDir = await mkdtemp(path.join(tmpdir(), 'xiangrui-hud-render-'));
  const originalConfigDir = process.env.CLAUDE_CONFIG_DIR;
  const originalNow = Date.now;
  const cachePath = path.join(tempConfigDir, 'plugins', 'xiangrui-hud', '.speed-cache.json');
