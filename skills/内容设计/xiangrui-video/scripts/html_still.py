...
# Keynote 玻璃语言公共件。2026-06-13: diagram 从规整玻璃卡换成深底 Rough.js 手绘框
# （design-system 去 AI 味 = 杀玻璃卡，但保留万涂幻象墨绿深底调性，不是纸白）；
# GLASS/BGGLOW 仍是 demo dark / word_card / ending / impact / editorial 等深底场景公共件。
...
def win_editorial(s, L):
    ...
    css = (
        ...
        f".eno{{font-family:'YSBTH';font-size:{int(cw * eno_fs)}px;line-height:.8;color:rgba(255,255,255,.16)}}"
        f".erow.on .eno{{color:{GREEN}}}"
        f".ecap{{font-family:'YSBTH';font-size:{int(cw * cap_fs)}px;letter-spacing:1px}}"
        f".edesc{{font-size:{int(cw * desc_fs)}px;color:rgba(255,255,255,.5);margin-top:8px}}"
...
"""
    ...
    # demo 场景走 demo_js；diagram 等卡片场景走 win_js（win_* 返回的第三个值）。
    all_js = (demo_js + "\n" + win_js).strip()
