...
- 聊天记录、群成员、系统消息（入群/移出通知）来自 macOS 微信 4.x 的本地 SQLCipher 数据库，由本仓库的 [vchat CLI](../../../cli/vchat) 解密和查询（也兼容其他本地微信数据访问工具）
...
微信升级后如果读不到新消息，重新跑 `vchat decrypt` 增量解密即可。
...
按 [`cli/vchat/README.md`](../../../cli/vchat/README.md) 安装（一句话贴给 Agent 即可）。首次使用需要 `sudo vchat setup` 解密微信本地库。仅支持 macOS + 微信 4.x。
...
这个项目永久免费使用。如果它帮到了你，欢迎[请祥瑞喝杯咖啡](https://pay.xiangruiai.com/?project=group-activity-base)，鼓励他继续维护并开源更多实用工具。赞赏完全自愿，不解锁任何功能。
