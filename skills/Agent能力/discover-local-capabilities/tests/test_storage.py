...
class ResolverDocumentTests(unittest.TestCase):
    ...
    def test_duplicate_resolver_ids_are_merged_without_losing_locations(self) -> None:
        paths = default_storage_paths(home=Path("/fixture/home"))
        document = build_private_resolver_document(
            paths,
            (
        ...
        self.assertEqual(
            document["records"],
            [{"resolver_id": "res_same", "exact_locations": ["/one", "/two"]}],
        )
...
class AtomicStorageWriterTests(unittest.TestCase):
    ...
    def test_symlink_roots_and_targets_are_rejected_without_following(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            linked_paths = default_storage_paths(home=home, local_root=linked)
            with self.assertRaises(ValueError):
                write_storage_bundle(linked_paths, _artifacts("linked"), ())
